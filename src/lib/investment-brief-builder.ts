// ============================================================
// Investment Readiness Brief builder.
// AI-written investment brief, infographic/box style,
// Clearview branding, concise not text-heavy.
//
// Shared by two callers: /api/investment-pitch (coach-triggered, from
// the coach dashboard) and /api/access-grant/[token] (an external party
// redeeming a coach-issued, revocable link). Both produce the exact same
// document from the exact same logic -- no second copy to drift out of
// sync. Throws a plain Error (with an optional .status) rather than
// returning a NextResponse, so each caller formats its own response.
// ============================================================
...
export async function buildInvestmentBrief(clientId: string): Promise<{ buffer: Buffer; fileName: string }> {
    ...
    if (!configRow) throw Object.assign(new Error('No financial model found. Set up the financial plan first.'), { status: 404 })
