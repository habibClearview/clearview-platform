...
describe('the chapters', () => {
  ...
  it('names all five methods, and never sells one', () => {
    ...
    expect(SRC).toContain('Three of these are canvases. Every decision on one page, in order, so you can see the whole picture at once — and see which pieces do not fit.')
    expect(SRC).toContain('The other two are instruments rather than canvases. Market Intelligence collects and verifies the numbers from each business. The Trade Liquidity Multiplier structures a reserve so that it unlocks many times its own value in trade credit.')
    expect(SRC).not.toContain('each method is a canvas')
