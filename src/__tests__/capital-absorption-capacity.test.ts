...
import { computeCapitalAbsorptionCapacity, type CACInputs } from '../lib/capital-absorption-capacity'
...
// being tested.
function makeInputs(overrides: Partial<CACInputs> = {}): CACInputs {
  return {
...
describe('computeCapitalAbsorptionCapacity — credit capacity', () => {
  it('REG: insufficient §SCP data returns null credit capacity, not a fabricated figure', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs({ scpDataConfidence: 'insufficient', stressClose_4wk: [] }))
  ...
  it('REG: an already cash-stressed business (negative stress floor) gets zero credit capacity with a clear reason', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs({ stressClose_4wk: [150_000, 90_000, -5_000, 60_000, 150_000, 150_000, 150_000, 150_000, 150_000, 150_000, 150_000, 150_000] }))
  ...
  it('REG: a healthy business gets a positive credit capacity with a low/high band bracketing it', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs())
  ...
  it('REG: "limited" data confidence applies a 0.7 discount relative to "reliable" with identical inputs otherwise', () => {
    const reliable = computeCapitalAbsorptionCapacity(makeInputs({ scpDataConfidence: 'reliable' }))
    const limited = computeCapitalAbsorptionCapacity(makeInputs({ scpDataConfidence: 'limited' }))
  ...
  it('REG: a supplier-financed business (cashConversionGap <= 0) does not divide by zero -- repaymentMonths floors at 1', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs({ cashConversionGapDays: -20 }))
    expect(r.credit.capacity).not.toBeNull()
  ...
  it('REG: a higher interest rate never produces a larger credit capacity than a lower one, all else equal', () => {
    const low = computeCapitalAbsorptionCapacity(makeInputs({ existingAnnualRate: 0.10 }))
    const high = computeCapitalAbsorptionCapacity(makeInputs({ existingAnnualRate: 0.30 }))
    expect(high.credit.capacity!).toBeLessThanOrEqual(low.credit.capacity!)
  })
})

describe('computeCapitalAbsorptionCapacity — grant capacity', () => {
  it('REG: a loss-making business (negative EBITDA margin) gets zero grant capacity', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs({ annualEbitda: -50_000 }))
  ...
  it('REG: governance below 2 gets zero grant capacity regardless of profitability', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs({ governanceScore: 1 }))
    expect(r.grant.capacity).toBe(0)
  ...
  it('REG: records completeness below 70% is flagged as an unmet condition, not silently ignored', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs({ recordsCompletenessPct: 40 }))
    expect(r.grant.conditions.some(c => /70%/.test(c) && /40%/.test(c))).toBe(true)
  })
})

describe('computeCapitalAbsorptionCapacity — equity capacity', () => {
  it('REG: a non-profitable business (NPAT <= 0) gets zero equity capacity', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs({ annualNpat: 0 }))
  ...
  it('REG: declining revenue trend discounts equity capacity by 40% relative to stable, all else equal', () => {
    const stable = computeCapitalAbsorptionCapacity(makeInputs({ revTrend: 'Stable' }))
    const declining = computeCapitalAbsorptionCapacity(makeInputs({ revTrend: 'Declining' }))
...
describe('computeCapitalAbsorptionCapacity — consignment capacity', () => {
  it('REG: no input shop unit identified returns null, not zero', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs({ inputShopUnit: null }))
  ...
  it('REG: a negative-margin input shop gets zero consignment capacity with a repricing prerequisite', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs({ inputShopUnit: { annualRevenue: 600_000, annualGrossProfit: -10_000 } }))
  ...
  it('REG: a healthy input shop gets monthly stock turnover times the cash conversion gap in months', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs({ cashConversionGapDays: 60 }))
  ...
  it('REG: a supplier-financed cash conversion gap floors ccgMonths at 1, not zero or negative', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs({ cashConversionGapDays: -30 }))
    const expected = 600_000 / 12 // ccgMonths floored to 1
    expect(r.consignment.capacity).toBeCloseTo(expected, 2)
  })
})

describe('computeCapitalAbsorptionCapacity — recoverable grant capacity', () => {
  ...
  it('REG: a configured repayableFraction is honoured exactly and not flagged as defaulted', () => {
    const r = computeCapitalAbsorptionCapacity(makeInputs({ repayableFraction: 0.25 }))
...
describe('computeCapitalAbsorptionCapacity — overall dataConfidence passthrough', () => {
  it('REG: the top-level dataConfidence matches whatever §SCP reported', () => {
    expect(computeCapitalAbsorptionCapacity(makeInputs({ scpDataConfidence: 'limited' })).dataConfidence).toBe('limited')
    expect(computeCapitalAbsorptionCapacity(makeInputs({ scpDataConfidence: 'reliable' })).dataConfidence).toBe('reliable')
  })
})
