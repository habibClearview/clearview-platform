...
> of Habib Onifade. Attribution is immutable in every rendering (removal is a licence breach).
...
## A. Party responsibilities matrix

### A1. Client / Funder / Programme (e.g. Tanager)
- **Throughout:** commissions the work; is the **final acceptor** ("Tanager has accepted the final
  deliverable" is a Zone 9 gate criterion). Receives the **Weekly Progress Report** (Fridays,
  LC-approved) and the milestone/phase reports. Escalation authority for RED status and for the
  non-negotiable pilot zone.
- **Pre-Engagement Diagnostic:** the **Country Representative is a mandatory attendee**; the signed
  record is filed with the funder. On weak answers, **the funder decides whether to proceed**.
- **Gates:** present at **all three Zone 9 diagnostics** (baseline/mid/close) as a joint session;
  **co-evaluator with the LC at Formal Handover** ("evaluators, not helpers").
...
- **Executive Director (ED):** Pre-Engagement Diagnostic **mandatory, non-delegable**; must name a
  ...
  **Sign-off authority at every gate** (pattern: *"CI drafts, LC reviews, ED signs"*). Confirms the
...
- **Board / Board Chair:** Chair (or delegate) is a **mandatory Pre-Engagement Diagnostic attendee**
  and signs. **Board approves the Scale Pathway Commitment** (DP08 gate).
- **Finance Lead (FL):** **mandatory at all three DP04 cost-mapping sessions**; must **update the
  financial model independently by end of Zone 4** (demonstrated live); owns quarterly model updates
  after close; performs **Independence Test 1** at Handover.
...
- **Setup:** completes Engagement Setup, links the Financial Model, sets permissions, duplicates
  capture templates, briefs and tests the field team.
- **At gates:** *"CI drafts, LC reviews, ED signs."* Compiles fieldwork, scores believability/urgency,
  prepares observation/debrief templates. **Trains the FL** to run the model independently (Zone 4).

### A5. Licensed Advisor
- A coach delivering under licence: occupies the **LC role for their own engagements**, scoped to
  their own clients, with **immutable IP/attribution**. *"This workbook is licensed, not sold."*
- **System implication:** the full coach surface, but tenant-scoped to the advisor's engagements, with
  a licence boundary and unremovable attribution.
...
**ED sign-off + LC approval**.

| DP | Core question | Key output(s) | Signal of genuine completion |
|---|---|---|---|
| **01 Service Reality Audit** | What do we actually deliver vs what exists only because of the grant? | Service Inventory (grant- vs market-logic), Hidden Cost Map, Stop/Pause/Redesign Register | LT will share the Stop/Pause/Redesign Register with its board; honest debate |
| **02 Customer & Problem Clarity** | Who will pay, for what problem, and how do we know? | ≥2 named segments with urgency, named budget holder, Three-Stage Adoption Test™, Budget Access Map | ≥1 customer-profile assumption changed by real conversations |
| **03 Value Proposition Architecture** | Why does this matter to this client, and can we prove it? | Client-tested 4-part value proposition per segment, signed | LT signs off without qualifying; the revision is shorter and more specific |
| **04 Commercial Viability Model** *(Clearview)* | Does this sustain the org with numbers that hold? | Working model (5 cost categories), price floor, ≥2 tiers, break-even, market reference (≥3 sources); operable by non-technical staff | A non-technical staff member updates the model and says "I can do this" |
| **05 Market Entry Design** | Right clients, message, channels, order? | Priority Client List, A/B-tested messaging (winner ≥50% higher response), Service Brief, live Pipeline Tracker + Launch Date | LT names the launch date without hesitation |
| **06 Identity & Partner Architecture** | Who are we, and who stands with us? | 2-sentence Commercial Identity Statement, Partner Type Map, Positioning; Commercial Vehicle option | LT uses the Identity Statement spontaneously |
| **07 Pilot & Learn** *(NON-NEGOTIABLE)* | Does the model work under real conditions? | 2 iterations × 2 real clients (Iter 1 LC-led, Iter 2 LSP-led), Revision Log, Comparative Analysis, Pilot Learning Summary, mid-point diagnostic to funder | LT makes a revision the coach did not identify; summary they're proud to share |
...
> Proposition / Outreach Process / Client Management Process / Commercial Identity, with a "30-day
> independent operation" demonstration). Pick one canonical set, configurable per engagement.
...
- **Per-DP working tools:** each DP = 9 components × 5 layers (*What it is · Why it matters · Coach
  ...
  Budget Access Map (DP02); Proposition Builder & Revision Log (DP03); **Financial Model build, tiers,
  break-even** (DP04, separate file); Priority Client List, **A/B Message Testing Log**, **Pipeline
...
### Reporting cadence & momentum protocol
- **Weekly Progress Report** (CI drafts Fri → LC approves → funder): Country · Week n/26 · Zones active
  · Sessions completed · Gates closed · Fieldwork completed · **Status GREEN/AMBER/RED** · Next-week
  plan. One page.
- **Momentum:** **GREEN** continue · **AMBER** (one missed session / delayed gate) → catch up within 5
...
## E. Frictions the interactive system removes

1. **Manual status juggling** → auto-derive DP status from component completion + evidence; auto-surface
   the top ⚠; **enforce "no zone opens until the prior gate closes."**
2. **Scattered evidence** → central store with real attachments, auto E-numbers, DP tagging,
   reliability/status, one-click "evidence for this gate / fit test."
